...
using AutoFixture;
using AutoMapper;
using lucas.OneDrive.Escritorio.Git.auctionApp.auctionApp.tests.Utils;
using MassTransit;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using Moq;

namespace auctionApp.auctionApp.tests
{
    public class AuctionControllerTests
    {
        ...
        public async Task GetAuctionById_WithValidGuid_ReturnsAuction()
        {
            ...
            // assert
            Assert.Equal(auction.Title, result.Value.Title); 
            Assert.IsType<ActionResult<AuctionDto>>(result); 
        }

        [Fact]
        public async Task CreateAuction_WithValidParameters_ReturnsAuction()
        {            
            // arrange
            var auction = _fixture.Build<Auction>().Without(x => x.Item).Create();
            auction.Item = _fixture.Build<Item>().Without(x => x.Auction).Create();
            auction.Seller = "tests"; 
            auction.Item.Category = _fixture.Build<Category>().Without(x => x.Items).Create();
            ...
            _auctionRepo.Setup(repo => repo.GetCategoryByName(It.IsAny<string>())).ReturnsAsync(auction.Item.Category);
